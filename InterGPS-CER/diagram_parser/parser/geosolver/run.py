...
from geosolver.grounding.ground_formula import ground_formulas, _ground_formula
...
import os.path
from parser.use_geosolver import image_to_graph_parse
...
def _annotated_unit_test(query):
    ...
    graph_parse = image_to_graph_parse(diagram)
    #修改部分，原diagram_to_graph_parse
    ...
    for number, sentence_words in question.sentence_words.iteritems():
        ...
        grounded_formulas = [_ground_formula(match_parse, formula, value_expr_formulas)
...
def _full_unit_test(combined_model, question, label_data):
    ...
    json.dump(diagram_parse_list, open(diagram_parse_path, 'wb'))          #dump编码json数据
