...
class seq_expand(nn.Module):
    def __init__(self):
        super(seq_expand, self).__init__()
        self.fc = nn.Linear(30, 120)
...
if __name__ == "__main__":
    ...
    D_net = discriminator()
    G_net = generator(1, 600)
    D_net = D_net.to(device)
    G_net = G_net.to(device)
    G_optimizer = torch.optim.Adam(G_net.parameters(), lr=0.002)
    D_optimizer = torch.optim.Adam(D_net.parameters(), lr=0.001)
    criterion = nn.BCELoss().to(device)

    ex_net = seq_expand().to(device)
    ex_optimizer = torch.optim.Adam(ex_net.parameters(), lr=0.0001)
    ...
    for epoch in range(1):
        for batch_num, (pb_id, _, problem, sol) in enumerate(trainloader):
            real_label = torch.ones(problem.size(0), 1).to(device)
            fake_label = torch.zeros(problem.size(0), 1).to(device)
            sol = sol.to(device)
            sol = ex_net(sol)
    ...
    for pid in range(2402, 3002):
        ...
        tz = torch.unsqueeze(tz, dim=0)
        fake_seq = G_net(torch.tensor(tz))
        fake_seq = fake_seq.detach().numpy()
